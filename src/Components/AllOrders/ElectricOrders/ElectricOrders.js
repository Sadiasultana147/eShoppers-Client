...
const ElectricOrders = () => {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:5000/electronicCollection/${_id}`)
            .then(res => res.json())
    ...
    const handleOrder = (e) => {
        ...
        const newOrder = { productName, productImage, productDetails, price, city, address, contact, userEmail, UserName };
    ...
    return (
        <div>
            <h1>Details of : {electronics?.name}</h1>


            <img src={electronics?.image} alt="" />
            <h5>{electronics?.features}</h5>
